...
export default {
  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "Please pay with your card", {
        dangerouslyUseHTMLString: true,
        showCancelButton: true,
        center: true,
        cancelButtonText: "Problems paying",
        confirmButtonText: "Success",
        showClose: false,
        beforeClose: (action, instance, done) => {
          if (action == "cancel") {
            alert("please contact us");
            clearInternal(this.timer);
            this.timer = null;
            done();
          } else {
            if (this.code == "200") {
      ...
      if (this.timer) {
        this.time = setInterval(async () => {
          let result = await this.$API.checkPayStatus(this.orderId);
